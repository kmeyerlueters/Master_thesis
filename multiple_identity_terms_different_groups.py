# count number of identity terms and how many there are in each group (multiple identity terms within the same group are NOT counted here)
import csv
import nltk
...
unwanted_phrases = [
    'a little white', 'african communities', 'american academy', 'american cancer society', 
...
def identify_and_save_arguments_with_identity_terms(row, csv_writer):
    ...
    found_terms = [word.lower() for word, _ in tagged_words if word.lower() in gender_terms | race_terms | neutral_terms]

    # Determine categories based on the found terms
    categories = set()
...
output_csv_path = '../multiple_identites_after_topic.csv'
